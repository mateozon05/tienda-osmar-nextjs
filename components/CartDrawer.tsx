...
export default function CartDrawer({ open, onClose }: Props) {
  ...
  async function handleWhatsApp() {
    if (items.length === 0 || loadingWA) return;
    setLoadingWA(true);

    try {
      // 1. Generar Excel compatible con SIPE
        ...
        .map((i) => {
          const typeLabel =
            i.purchaseType === "bulto"
              ...
              : "Unidad";
          return `• ${i.quantity}× ${i.name} (${typeLabel}) — $${(i.price * i.quantity).toLocaleString("es-AR")}`;
        })
        .join("\n");
      ...
      // 3. Descargar el Excel automáticamente
      if (data.excel) {
      ...
      console.error("Error generando pedido WhatsApp:", err);
      // Fallback: abrir WhatsApp sin Excel
      const lines = items
      ...
      <div className={`cart-drawer${open ? " open" : ""}`}>
        ...
        {items.length > 0 && (
          <div className="cart-foot">
            ...
            <p style={{ fontSize: "0.72rem", color: "var(--muted)", textAlign: "center", margin: "6px 0 0" }}>
              Se abre WhatsApp y se descarga el Excel para importar en SIPE
            </p>
          </div>
        )}
      </div>
